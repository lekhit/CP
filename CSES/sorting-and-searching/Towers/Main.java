...
public class Main {
    static void solve() throws IOException {
        // Your solution here
        int n = in.nextInt(), minTowers = 0;
        int[] arr = readArray(n);
        MultiSet<Integer> towers = new MultiSet<>();
        for (int i : arr) {
            if (towers.higher(i) != null) {
                towers.remove(towers.higher(i));
            }
            towers.add(i);
            minTowers = Math.max(towers.size(), minTowers);
        }
        out.print(minTowers);
    }
