...
public class Main {
    ...
    static void solve() throws IOException {
        ...
        int initial_count=1;
        for(int i=0;i<n;i++){
            initial_count+= pos[arr[i]] < pos[arr[i]-1]?1:0;
        }
        ...
        while(m-- > 0){
            ...
            Set<Integer> changedValues = new HashSet<Integer>();
            changedValues.add(val1);
            changedValues.add(val1+1);
            changedValues.add(val2);
            changedValues.add(val2+1);
            for(int i: changedValues){
                if(i>=0 && i<=n){
                    initial_count-= pos[i]<pos[i-1]?1:0;
                }
            }
            int temp=arr[a];
            arr[a]=arr[b];
