...
int main() {
    ...
    printf("Turned off write permission on '%s'; permission code 400...\n", filename);

    // Step 4: Attempt the second write using the existing file descriptor.
    // This is the test for Complete Mediation. The kernel should, but likely
    // won't, re-check the file's permissions on disk.
    auto bytes_written = write(fd, second_write_str, strlen(second_write_str));

    // Step 5: Analyze the result.
    if (bytes_written < 0) {
        // This would be the correct behavior under Complete Mediation.
        perror("Second write failed");
        printf("\nThis system appears to honor the Principle of Complete Mediation for this operation.\n");
    } else {
        // This is the expected outcome on Linux.
        printf("Second write successful...\n");
        printf("\nThis system does NOT honor the Principle of Complete Mediation.\n");
        printf("The kernel did not re-check permissions for the second write call.\n");
    }
